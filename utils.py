        ...
        for e in range(num_env + 1):
            if (e <= num_env - 1):
                ...
                if args.net == 't2tvit7_ours':
                    model_list.append(T2t_vit_7_feature(num_classes=2, final_k=variance_opt['final_k']))
                else:
                    try:
                        model_list.append(ResidualNet("ImageNet", 18, num_classes=2, att_type='CBAM', split_layer=variance_opt['split_layer']))
                    except:
...
def load_NICO(dataroot, balance_factor=1, config=None):
    ...
    for file_name in all_file_name:
        ...
        all_data.append(Image.open(os.path.join(dataroot, file_name)).convert('L'))
...
class NICO_dataset_env_posNeg(torch.utils.data.Dataset):
    def __init__(self, all_data, all_label, all_context, env_idx, transform=None, label2train=None):
        super(NICO_dataset_env_posNeg, self).__init__()
        self.all_data =  all_data
        self.all_label = all_label
        self.all_context = all_context
        self.transform = transform
        ...
        label_np=np.array(self.all_label).astype('uint8')
        self.pos_idx=np.where(label_np==1)[0]
        self.neg_idx=np.where(label_np==0)[0]
        self.pos_len=self.pos_idx.shape[0]
        self.neg_len=self.neg_idx.shape[0]
        self.pos_cur=0
        self.neg_cur=0
    ...
    def __getitem__(self, item):
        ...
        if posNeg==0:
            item=self.pos_idx[self.pos_cur%self.pos_len]
            self.pos_cur+=1
        else:
            item=self.neg_idx[self.neg_cur%self.neg_len]
            self.neg_cur+=1
...
class init_training_dataloader():
    ...
    def get_env_dataloader(self, config, batch_size=16, num_workers=1, shuffle=True, pre_split=None):
        ...
        for env_idx in range(n_env):
            ...
            training_dataset.append(NICO_dataset_env_posNeg(image_[env_idx], label_[env_idx], context_[env_idx], env_idx, transform=self.transform, label2train=training_dataset_all.label2train))
        training_loader = []
...
def load_model(net, path):
    if isinstance(net, list):
        ...
        for idx, model in enumerate(net):
            items=loaded_model[idx].keys()
            loaded={}
            for item in items:
                loaded[item]=loaded_model[idx][item]
                # loaded[item[7:]]=loaded_model[idx][item]
